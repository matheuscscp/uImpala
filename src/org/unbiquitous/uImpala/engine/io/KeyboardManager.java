...
public class KeyboardManager implements InputManager {
	...
	public IOResource alloc() {
		...
		Iterator<Entry<String, KeyboardSource>> i = availableKeyboards
				.entrySet().iterator();
	...
	private void updateLists() {
		// get all drivers
		List<DriverData> currentDrivers = gateway
				.listDrivers("KEYBOARD_DRIVER");
		...
		for (DriverData driver : currentDrivers) {
			...
			if (availableKeyboards.get(id) == null
					&& busyKeyboards.get(id) == null)
	...
	private void checkDisconnected(HashMap<String, KeyboardSource> container,
			HashSet<String> currentDriversIDs) {
		for (Iterator<Entry<String, KeyboardSource>> i = container.entrySet()
				.iterator(); i.hasNext();) {
			Entry<String, KeyboardSource> e = i.next();
	...
	public List<IOResource> list() {
		return (List<IOResource>) busyKeyboards.clone();
	}
}
